...
class ManualDrawWindow(QWidget):
    def __init__(self, parent_viewer, current_slice_img, dicom_path, slice_index):
        super().__init__()
        self.setWindowTitle("Manual Drawing Tool")
        self.setGeometry(300, 300, 800, 600)
        self.setStyleSheet("background-color: #F0F8FF;")
        self.parent_viewer = parent_viewer
        self.original_slice_img = current_slice_img.copy()  # Store original for reset
        self.current_slice_img = current_slice_img  # Working copy for adjustments
        self.dicom_path = dicom_path
        self.slice_index = slice_index
        self.mask_rgb = np.zeros_like(current_slice_img, dtype=np.uint8)
        self.current_color = [255, 0, 0]
        self.contrast_factor = 1.0  # Default contrast
        self.brightness_offset = 0  # Default brightness
        self.initUI()
    ...
    def initUI(self):
        ...
        self.eraser_btn.setIcon(app.style().standardIcon(QStyle.SP_LineEditClearButton))
        ...
        self.load_auto_btn.setIcon(app.style().standardIcon(QStyle.SP_MessageBoxInformation))
    ...
    def update_slice(self, value, slice_label):
        ...
        try:
            # Update slice data from parent viewer, ensuring 3D RGB
            ...
            slice_label.setText(f"Slice: {self.slice_index + 1}/{len(self.parent_viewer.dicom_images)}")
